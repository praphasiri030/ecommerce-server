...
exports.saveOrder = async (req, res) => {
  try {
    const { clientSecret, cardNumber } = req.body.payload;

    if (!clientSecret) {
      return res
        .status(400)
        .json({ ok: false, message: "Missing clientSecret" });
    }

    const amountFromSecret = clientSecret.split("_amt_")[1].split("_id_")[0];
    const amountSatang = Number(amountFromSecret);

    const id = "mock_charge_" + Math.random().toString(36).substr(2, 9);
    ...
    const amountTHB = Math.floor(amountSatang / 100);
    // Create a new Order
